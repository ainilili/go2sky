...
.PHONY: test
test:
	go test -v -race -cover `go list ./... | grep -v github.com/ainilili/go2sky/reporter/grpc`
...
mock-gen:
	cd $(GRPC_PATH)/register && \
	  ...
	  mockgen github.com/ainilili/go2sky/reporter/grpc/register RegisterClient > mock_register/Register.mock.go && \
	  mockgen github.com/ainilili/go2sky/reporter/grpc/register ServiceInstancePingClient > mock_register/InstancePing.mock.go
	cd $(GRPC_PATH)/language-agent-v2 && \
    	  mkdir -p mock_trace && \
    	  mockgen github.com/ainilili/go2sky/reporter/grpc/language-agent-v2 TraceSegmentReportServiceClient > mock_trace/trace.mock.go
